...
export class App {
  ...
  configureRouter(config: RouterConfiguration, router: Router) {
    ...
    config.map([{ route: '/signin/device/failed', name: 'DeviceFailed', title: 'Device Authorize Failed', moduleId: PLATFORM.moduleName('Clients/Device/Failed') }]);
    config.map([{ route: '/signin/device/success', name: 'DeviceSuccess', title: 'Device Authorize Succeeded', moduleId: PLATFORM.moduleName('Clients/Device/Success') }]);
